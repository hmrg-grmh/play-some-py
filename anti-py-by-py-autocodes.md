...
好，一边查一边干！（一边查是因为我不会 PY ，也正因此才有了这份……笔记？）
...
> `lambda` ：别担心，小伙子！想一想这个美妙的结构： `(lambda a (a a))` （这个是 Scheme 里的写法，等价的 Python 写法是这样： `(lambda a: a(a))` 。怎么样？是不是 Scheme 比较牛逼？😶）（另外这个结构来自 
*Y 组合子* 。）
...
这里我想多说一句：**其实也不用引入，单纯用 `lambda` 我能做到手动实现一个差不多的东西！**看：
...
上面的部分不需要掌握，自己把最下面的参数随便换一换执行看看结果就好。**这部分，我只是想说明一下聚合函数是可以只通过 `lambda` 定义的。。。**
